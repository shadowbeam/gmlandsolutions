...
function go_child_menu() 
{
	...
	else //We do not have a cached file, create a new one.
	{
		...
		if (count($filenames) != 0)
		{
				...
				if (!($file == '.' || $file == '..' || is_dir(GSDATAPAGESPATH.$file) || $file == '.htaccess'))
				{
					...
					if ($XMLdata->private != 'Y' and strcmp($XMLdata->url,$active_parent)==0)
					{
						...
						if ($PRETTYURLS==1)
						{
							...
							{
								$childmenuparent='<li id="parent"'.$current.'><a href="'.$XMLdata->url.'">'.$XMLdata->menu.'</a></li>';
							}
							else
							{
								$childmenuparent='<li id="parent"'.$current.'><a href="'.$XMLdata->url.'">'.$XMLdata->title.'</a></li>';
							}
						}
						else
						{
					...
					elseif ($XMLdata->private != 'Y' and strcmp($XMLdata->parent,$active_parent)==0) //If child.
					{
						...
						if ($PRETTYURLS==1)
						{
							if (strlen($XMLdata->menu)>0)
							{
								$childmenuarray[]='<li'.$current.'><a href="'.$XMLdata->url.'">'.$XMLdata->menu.'</a></li>';
							}
							else
							{
								$childmenuarray[]='<li'.$current.'><a href="'.$XMLdata->url.'">'.$XMLdata->title.'</a></li>';
							}
						}
						else
						{
			...
			{
				$thismenu='<ul class="list-group" id="child_menu">'.$childmenuparent.$childmenu.'</ul>';
